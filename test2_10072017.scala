...
object Test extends Serializable {
    ...
    def main(args: Array[String]) {
        ...
        */
        ...
        dataset.foreach(line => buf ++= (Vectors.dense(line._3).toArray ++ getIndex(Ui, line._1).toArray ++ Vi(line._2).toArray).mkString("\t") ++ "\n")
        println(buf.toString.length)
        val arrayData = buf.toString
        */
        ...
        val rdd = sc.parallelize(arrayData).map(Row.fromSeq(_))
